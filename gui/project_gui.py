...
class SearchApplication:
    ...
    def initial_map(self):
        ...
        self.map_canvas.bind("<Button-1>", lambda event, x="initial": self.map_on_click(event, x))
        
        self.map_image = self.map_canvas.create_image(5,5, image=tk_img, anchor=tkinter.NW)
        self.map_canvas.image = tk_img

    def map_on_click(self, event, name):
        name = name.replace(" ", "%20")
        rela_path = "../data/map/{}.html".format(name)
        path = 'file:///' + os.path.realpath(rela_path)
        path = path.replace('\\', '/')
        #path = 'file:///C:/Users/qingy/Documents/usc/DSCI551/Project/data/map/{}.html'.format(name)
    ...
    def update_map(self, name):
        ...
        img = img.resize((760, 400), Image.ANTIALIAS)
        tk_img = ImageTk.PhotoImage(img)
        self.map_canvas.bind("<Button-1>", lambda event, x=name: self.map_on_click(event, x))
